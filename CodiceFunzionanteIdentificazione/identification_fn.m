function res = identification_fn(file_to_load, folder, win_size)
try
    % addpath([curr_fold, '\preprocessedData\Test_2022_12_20']);
    load(file_to_load);
    addpath('functions');
    
    iteration = ceil(size(data, 1) / win_size);
    win = win_size; % # of samples of the window: 60000/fs / 60 = 10 min (600 sec) --> da ripetere 7 volte per coprire l'intero segnale
    for ide_singola = 1:iteration
        clearvars -except ide_singola win data curr_fold folder win_size file_to_load
        disp(["Starting loop execution with ide_singola", ide_singola]);
        tic;
        ...
        disp(ceil(win*(ide_singola-1)+1));
        disp(min(floor(win*(ide_singola)),size(data,1)));
    
        signal = data(ceil(win*(ide_singola-1)+1):min(floor(win*(ide_singola)),size(data,1)),2:end); % acc data (n.samples x n.channels)
        ...
        for i2=ssi.n_min:ssi.n_step:ssi.n_max
            ...
            disp([n_block, fs, i2]);
            ID{count,1}=SSI(signal,n_block,fs,i2);
            count=count+1;
        end
        ...
        filename = fullfile(folder, sprintf("Dendogram_%d.png", ide_singola));
        saveas(gcf, filename)
        ...
        for n_cluster=1:q_clustz %inizio a valutare il gruppo di CLUST di appartenenza
            ...
            if size(q,1)>= stab.Nmin_stab
                ...
                k = find ( MAC_iDiag == 0); % find : cerca l’ indice per cui x `e nullo
                MAC_iDiag(k) = NaN;
        ...
        filename = fullfile(folder, sprintf('results_identification_filt_%d',ide_singola));
        save(filename,'-v7.3');
    ...
    filename = fullfile(folder, "Frequency_SystemOrder.png");
    saveas(gcf, filename);
    ...
    filename = fullfile(folder, 'Frequency_DampingRatio.png');
